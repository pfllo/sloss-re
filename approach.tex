...
\red{(YY: "moderate extra training time" did not conform to the experiment result, drop?)}
...
If the subject (or object) set of relation $r_1$ in \KB has a large intersection with those in $r_2$, then we consider $r_1$ and $r_2$ to have the same expected subject (or object) type. \red{(YY: change $R_1$ and $R_2$ to $r_1$ and $r_2$)}
...
\red{(YY: change the $\bm{C}^{tso}$ to no order)}
...
\red{(YY: change to no order version)}
 respectively.
...
\red{(YY: change $\bm{X}$ to $\bm{X}=\{X_1, X_2,...,X_n\}$)}
...
We thereby define three semantic loss terms: $m^{ts}L^{s}(\bm{C}^{ts}, \bm{p})$, $m^{to}L^{s}(\bm{C}^{to}, \bm{p})$, $m^{tso}L^{s}(\bm{C}^{tso}, \bm{p})$, where $m^{ts}$, $m^{to}$, $m^{tso}$ are 0-1 masks that are set to 1 when $subj_1=subj_2$, $obj_1=obj_2$, $subj_1=obj_2$ respectively.
\red{(YY: change the type constraints from 2R to R variables)}
...
\red{(YY:add the $p$ generation of Type and Cardinality Constraints)}
...
\red{(YY: add $\lambda^{*}$, and change $0 \leq i, j \leq batch\_size$ to $0 \leq i < j < batch\_size$)}
