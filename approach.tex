...
For any combination of two entity pairs in a batch, we use $p^{t}_{i}$ and $p^{c}_{i}$ in Eq.~(\ref{Eq:compute_p}) to denote the $i^{th}$ value of probability vector $\bm{p}$ for type and cardinality constraints in Eq.~(\ref{seq:semantic_loss}), respectively.
\begin{equation}
\label{Eq:compute_p}
...
\end{equation}
where $\bm{p'}_1$ and $\bm{p'}_2$ represents the relation distribution outputs of our base model for the two entity pairs,
