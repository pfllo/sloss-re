...
Thus we finally get 5 sub-categories of constraints set $\bm{C^{\Phi}}$, where $\bm{\Phi}=\{ts, to, tso, cs, co\}$
...
Here $\lambda^{k}$ denotes the corresponding weight coefficient and $m^{k}$ is a 0-1 mask which are set to 1 when the two entity pairs satisfies the conditions of $\bm{C}^{k}$.
